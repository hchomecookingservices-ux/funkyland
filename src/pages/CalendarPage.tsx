...
export default function CalendarPage() {
  const { events, addEvent, updateEventStatus, addInvoice, categories, services, members, exportToCSV, businessProfile } = usePlayZone();
  ...
  const handleWhatsAppEvent = (event: BookingEvent) => {
    ...
    window.open(formatWhatsAppLink(event.phoneNumber, message), '_blank');
  };
          ...
          {sortedEvents.map((event, i) => (
            ...
            >
              ...
              <div className="bg-slate-50 p-4 border-t border-slate-100 flex items-center justify-between">
                <button 
                  onClick={() => updateEventStatus(event.id, event.status === 'confirmed' ? 'completed' : 'confirmed')}
                  className="text-[10px] font-black text-secondary uppercase tracking-widest hover:underline"
                >
                  {event.status === 'confirmed' ? 'Mark as Completed' : 'Confirm Booking'}
                </button>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400">{event.phoneNumber}</span>
                  <button className="p-2 bg-white rounded-xl text-slate-400 hover:text-secondary hover:bg-secondary/10 transition-all shadow-sm">
            ...
            >
              ...
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black text-slate-800 flex items-center gap-3">
                  <PartyPopper className="text-secondary" size={28} /> Schedule New Event
                </h2>
